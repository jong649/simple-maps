...
name = paletteName
lang = "display name"
layer = "proper layer name"
metric = true false
*/
function loadGradientLegendCWS (name, lang, layer, metric, __units, initDate) {
    if(!name) { var name = "mean"; }
    var unit = (metric == 'metric' ? 'true' : 'false');
    var callback = function (title, date) {
	var showLegendHtml = function (jSON) {
	    var palette = eval('(' + jSON.responseText + ')');
	    var units = __units; //palette.ColorPalette['units'];
	    var size = (palette.ColorPalette['size']);
	    var titleLang = (lang ? lang : title); 
	    var legend = new LegendPalette(palette.ColorPalette, "<div style='white-space:nowrap'>"+titleLang+"<span style='margin-left: 14px;'>"+date+"</span></div>", units, size, root);
	    legend.showGradient();
...
LegendPalette.prototype.buildSmoothLegend = function () {
    ...
    if(rate<=0) rate=1;
    for(i=0;i<size;i=i+rate) {	
	var upper=Math.round(legend.table[i].upper['level']*1);
...
var tableView = 
	'<div>' +
		'<div>' + 
		'<img src="http://clients.customweather.com/demos/future_radar/animation_framework/cw_demo/imageProxy.php?client=alex&client_password=trust_me&BBOX=iowa&format=image/png&LAYERS=legend&legend.stand_alone=true&palette.name=mean.cpt&width=400&height=50" alt="DemoPalette" width="400" height="50"/>' +
		'</div>' +
	'</div>';
...
LegendPalette.prototype.checkUnits = function(units_raw) {
    ...
    if(units_raw.startsWith("cws_")){
	return units_raw.substring(4);
    }
