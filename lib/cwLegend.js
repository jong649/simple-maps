...
function cwLoadLegend(name, unit, callback) {
    ...
    xttp.open("GET", "../maptiles/layer_info/xmlFEED.php?client=tile_demo2&apikey=zr_FX9XukUgZetYI9JIINzkhlcktJjS3yqEV3rKXuGFiNCDT1gtcCGKuFkc1jQzl&product=palette&palette.name="+name+"&metric="+unit+"&format=json", true);    
    xttp.send();
};
...
function cwLoadCapabilities(name, callback) {
    ...
    xttp.open("GET", "../maptiles/layer_info/imageFEED.php?REQUEST=capabilities&capdoc.extended=true&LAYERS="+name, true);
    xttp.send();
};
