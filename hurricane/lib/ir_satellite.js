...
function init(){
	...
	queryString = new QueryData();

if(CW_API_KEY==null){
	alert("Must provide a valid CW_API_KEY in lib/ir_satellite.js");
	return;
}
...
var base = BING_API_KEY==null ? mapbox :  bing;
...
for(ii=0;ii<numFrames;ii++){
    ...
    var tileSource = new ol.source.XYZ({
        ...
        url: 'http://maps-beta.customweather.com/smt/'+CW_API_KEY+'/'+layerName+'/' + ts + '/{z}/{x}/{y}.png' + ("?" + layerQueryString )
    });
...
function startAnimate(){
	...
	if(this.counter>this.numFrames){
	    ...
	    this.intervalTimerId = setInterval('updateAnimation()', this.slow);
	}
    }
}
...
function updateAnimationExplicit(explicitCall){
    var currentFrame = this.iter%(this.tileCollection.length-1);
    ...
    for(i=1; i<this.tileCollection.length; i++){
	if(i==(currentFrame+1)){ // +1 because base is in frame
	    this.tileCollection[i].setOpacity(.75);
	    this.tileCollection[i].setVisible(true);	
	    continue;
	}
	this.tileCollection[i].setVisible(false);	
    }
